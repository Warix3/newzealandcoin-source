...
#if defined(NDEBUG)
# error "NewZealandCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa7746629b6fe7138631d232b8bcd234865989cfee657aedf07941691aa89032a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // NewZealandCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "NewZealandCoin Signed Message:\n";
...
{
    // NewZealandCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 80 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 201480000 * COIN;
            }

    // Subsidy is cut in half every 839500 blocks
    nSubsidy >>= (nHeight / 839500);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NewZealandCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NewZealandCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1469023507, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "New Zealand Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 80 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048d2c67cb7e6d8df4dd30766c6dff0956f4efcb8f4430c85b76a43d8ed0a0e0b2ba7ebcb33b6e9831c13075ba1ce1da6cc6ca81a0c82100de3a909d97206c173e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1469023507;
        ...
        assert(block.hashMerkleRoot == uint256("0xff36e09955e90bfd38ddfc5879867edd30b229550f27e161e0ab2827a70a4c40"));
...
unsigned char pchMessageStart[4] = { 0x69, 0x2a, 0xb5, 0x47 }; // NewZealandCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NewZealandCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NewZealandCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NewZealandCoinMiner(CWallet *pwallet)
{
    printf("NewZealandCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("newzealandcoin-miner");
    ...
    try { loop {
        ...
        printf("Running NewZealandCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NewZealandCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NewZealandCoinMiner, pwallet));
}
